...
Measured results depend on the selected FPGA device, tool version, constraints, and generic values. The reported implementation resutls were derived from a top file that only instantiates the `spi_master` paired with a constraint file that only exposes the `W5` on-board clock for the `xc7a35tcpg236-1` board. Results may differ when implemented otherwise.
...
| Tool Version | `Vivado v2025.2 (64-bit)` |
| Clock Constraint | `100MHz on-board clock` |
| Important Configuration | `No constraint except clock` |
| Report Type | `Post-Implementation` |

### Timing

| Metric | Value |
| ------ | ----- |
| Worst Negative Slack (WNS) | `5.873 ns` |
| Worst Hold Slack (WHS) | `4.500 ns` |
...
- The receive path has only one storage word. An unacknowledged word is replaced by a later completion, with `o_rx_overrun` asserted; the overwritten word is not recoverable. Therefore module requires a controller that will perform the flow control, read the SPI responses returned, and possibly buffer them.  
